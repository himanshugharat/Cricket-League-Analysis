...
public class DataSorting {
    ...
    public static String sortData(String parameter, Map<String, LeagueDAO> leagueDAOMap) {
        ...
        switch (parameter) {
            case "AVG":
                comparator = Comparator.comparing(LeagueDAO::getBattingAverageScore).reversed();
                break;
            case "STRIKE_RATE":
            ...
            case "AVG_STRIKE_RATE":
                comparator = Comparator.comparing(LeagueDAO::getBattingAverageScore)
                        .thenComparing(LeagueDAO::getStrikeRate).reversed();
                break;
            case "RUN_AVG":
                comparator = Comparator.comparing(LeagueDAO::getBattingAverageScore)
            ...
            case "BOWLING_SR_4W_5W":
                comparator = Comparator.comparing(LeagueDAO::getStrikeRate)
                        .thenComparing(LeagueDAO::getFoursWickets)
                        .thenComparing(LeagueDAO::getFiveWickets).reversed();
                break;
            case "BOWLING_SR_AVG":
                comparator = Comparator.comparing(LeagueDAO::getStrikeRate)
            ...
            case "BOWLING_WKTS_AVG":
                ...
                break;
            case "SIX_AVG":
                comparator = Comparator.comparing(LeagueDAO::getBattingAverageScore).thenComparing(LeagueDAO::getNoOfHundreds).reversed();
                break;
        }
        ...
        switch (parameter) {
            ...
            case "AVG":
                comparator = Comparator.comparing(LeagueDAO::getBowlingAverageScore)
                        .thenComparing(LeagueDAO::getBattingAverageScore).reversed();
                break;
        }
        leagueList = new ArrayList<>(leagueDAOMap.values());
        leagueList.sort(comparator);
